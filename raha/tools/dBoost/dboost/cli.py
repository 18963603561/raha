...
def get_stdin_parser():
    ...
    # dBoost 会读取 Raha 生成的临时 CSV，固定使用 UTF-8 可避免中文表头在 Windows 默认 GBK 下解码失败。
    parser.add_argument("input", nargs='?', default = "-", type = argparse.FileType('r', encoding='utf-8'),
    ...
    parser.add_argument("--train-with", dest = "trainwith", metavar = "input",
                        # 训练集和测试集编码必须一致，否则同一次检测中可能出现解码口径不一致。
                        action = "store", default = None, type = argparse.FileType('r', encoding='utf-8'),
